...
-  Twitch VODs
-  Twitch Clips
-  chat for VODs and Clips, in either a [JSON with all the original information](https://github.com/lay295/TwitchDownloader/files/13495494/ExampleMoonMoonJsonFile.json), a browser HTML file, or a [plain text file](https://github.com/lay295/TwitchDownloader/files/13495523/ExampleMoonMoonTextFile.txt)
...
- Queue up multiple /render jobs to run simultaneously
- Create a list of  jobs from a list of vod/clip links
- Search for and  multiple VODs/clips from any streamer without leaving the app
...
With the CLI, it is possible to automate video processing using external scripts. For example, you could copy-paste the following code into a `.bat` file on Windows to  a VOD and its chat, and then render the chat, all from a single input.
...
1. Go to [](https://github.com/lay295/TwitchDownloader//) and  the latest version for Windows or [build from source](#building-from-source).
...
4. If you do not have FFmpeg, you can install it via [Chocolatey package manager](https://community.chocolatey.org/), or you can get it as a standalone file from [ffmpeg.org](https://ffmpeg.org/.html) or by using TwitchDownloaderCLI:

```
TwitchDownloaderCLI.exe ffmpeg --
```
...
3. Go to [](https://github.com/lay295/TwitchDownloader//) and  the latest binary for Linux, grab the [AUR Package](https://aur.archlinux.org/packages/twitch-downloader-bin/) for Arch Linux, or [build from source](#building-from-source).
...
7. a) If you do not have FFmpeg, you should install it system-wide via your distro package manager, however you can also get it as a standalone file from [ffmpeg.org](https://ffmpeg.org/.html) or by using TwitchDownloaderCLI:

```
./TwitchDownloaderCLI ffmpeg --
```
...
1. If your device has an Apple Silicon M-series processor, ensure that you  the arm64 binary, however if you would like to use the x64 binary on Apple Silicon it must be run via a terminal session running under Rosetta 2:

```
arch -x86_64 zsh
```

2. Go to [](https://github.com/lay295/TwitchDownloader//) and  the latest binary for MacOS or [build from source](#building-from-source).
...
6. a) If you do not have FFmpeg, you can install it system-wide via the [Homebrew package manager](https://brew.sh/), or you can get it as a standalone file from [ffmpeg.org](https://ffmpeg.org/.html) or by using TwitchDownloaderCLI:
...
- [.NET 10.0.x SDK](https://dotnet.microsoft.com/en-us//dotnet/10.0)
...
```
cd TwitchDownloaderWPF/bin//net10.0-windows/publish/win-x64
```

5. b) Navigate to the CLI build folder:

```
cd TwitchDownloaderCLI/bin//net10.0/publish
```
...
Chat Renders are encoded and Video  are finalized with [FFmpeg](https://ffmpeg.org/) © The FFmpeg developers.
...
FFmpeg binaries fetched are runtime are downloaded using [![download button](https://i.imgur.com/3Dm4p65.png)](https://tinyurl.com/2yxcepj2) © Xabe.
